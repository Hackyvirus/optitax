...
const host = "0.0.0.0"; // ← required for Render/Railway to detect the port
...
app.prepare().then(() => {
  ...
  const io = new Server(httpServer, {
    ...
    cors: {
      origin: process.env.NEXT_PUBLIC_URL || (dev ? "http://localhost:3000" : false),
      methods: ["GET", "POST"],
      credentials: true,
    },
  });
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", async (data: { text: string; roomId: string; receiverId?: string }) => {
      try {
        ...
        const saved = await res.json() as { message?: { _id?: string; [key: string]: unknown } };
        if (!saved.message) return;

        const payload = { ...saved.message, _id: saved.message._id || Date.now().toString() };
    ...
    socket.on("typing_start", (data: { roomId: string }) => {
      socket.to(`room:${data.roomId}`).emit("user_typing", { userId, role, roomId: data.roomId });
    });
    socket.on("typing_stop", (data: { roomId: string }) => {
      socket.to(`room:${data.roomId}`).emit("user_stop_typing", { userId, roomId: data.roomId });
    });
    socket.on("mark_read", (data: { roomId: string }) => {
      socket.to(`room:${data.roomId}`).emit("messages_read", { roomId: data.roomId, by: userId });
    });
  ...
  // Listen on 0.0.0.0 so Render/Railway can detect the open port
  httpServer.listen(port, host, () => {
    console.log(`> Ready on http://${host}:${port} (${dev ? "dev" : "production"})`);
  });
