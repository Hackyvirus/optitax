...
// Projects loaded from MongoDB via /api/projects
...
function mapProject(p: Record<string, unknown>): Project {
  ...
  return {
    ...
    messages: ((p.comments as Record<string,unknown>[]) || []).map((c, i) => ({
      id:         String(c._id || i),
      author:     String(c.authorName || "Team"),
      authorRole: (c.authorRole as "admin" | "employee" | "client") || "admin",
      content:    String(c.content || ""),
      createdAt:  String(c.createdAt || ""),
    })),
    createdBy:  String(p.createdBy || "admin"),
...
export default function AdminProjectPage() {
  ...
  useEffect(() => {
    fetch("/api/projects")
      .then(r => r.json())
      .then(d => {
        setProjects((d.projects || []).map(mapProject));
    ...
    <div style={{ padding:"32px 36px", minHeight:"100%", fontFamily:"'Segoe UI',sans-serif" }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:28, gap:16, flexWrap:"wrap" as const }}>
        <div>
          ...
          <p style={{ color:"#64748b", fontSize:15, margin:0 }}>Manage all client projects and assign team members.</p>
        </div>
      ...
      {selected && (
        <ProjectPanel project={selected} onClose={() => setSelected(null)} role="admin" />
      )}
