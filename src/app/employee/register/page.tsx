...
const cfg   = ROLE_CONFIG.employee;
const STEPS = ["Email","Personal","Work Details","Review"];
const DEPTS = ["GST & Indirect Tax","Direct Tax & ITR","Customs & Trade","Legal & Compliance","Accounts & Audit","Management"];
const SPECS = ["GST","Customs","Brand Rate","MOOWR","SEZ","ITR","Legal","Audit","Company Law"];
...
type F = { email:string; emailOtp:string; emailVerified:boolean; firstName:string; lastName:string; phone:string; password:string; confirmPassword:string; department:string; designation:string; inviteCode:string; specializations:string[] };
const INIT: F = { email:"", emailOtp:"", emailVerified:false, firstName:"", lastName:"", phone:"", password:"", confirmPassword:"", department:"", designation:"", inviteCode:"", specializations:[] };
...
export default function EmployeeRegister() {
  ...
  const validate = () => {
    ...
    if (step===2) { if (!f.department) errs.department="Required"; if (!f.designation) errs.designation="Required"; if (!f.inviteCode.trim()) errs.inviteCode="Invite code is required"; }
    setErrors(errs);
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/auth/register", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({...f, role:"employee"}) });
      const data = await res.json();
      if (!res.ok) { setGlobalErr(data.error||"Failed"); return; }
      window.location.href = data.redirect || "/dashboard/employee";
    } catch { setGlobalErr("Something went wrong."); }
    finally { setLoading(false); }
  };

  const toggleSpec = (sp: string) => set("specializations", f.specializations.includes(sp) ? f.specializations.filter(x=>x!==sp) : [...f.specializations, sp]);
  ...
  return (
    <AuthShell cfg={cfg} mode="register">
      <div style={{marginBottom:20}}>
        ...
        <h1 style={{fontSize:20,fontWeight:700,color:"#1e293b",margin:"0 0 4px"}}>Employee registration</h1>
      ...
      {step===0 && (
        <div>
          <div style={{padding:"12px 16px",background:"#fef3c7",border:"1px solid #fde68a",borderRadius:10,marginBottom:14}}>
            <div style={{fontSize:13,fontWeight:600,color:"#92400e",marginBottom:2}}>👤 Employee registration</div>
            <div style={{fontSize:12,color:"#b45309"}}>You need an invite code from your manager to register.</div>
          </div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:4}}>
            <input type="email" placeholder="you@optitax.in" value={f.email} disabled={f.emailVerified} onChange={e=>set("email",e.target.value)} style={f.emailVerified?{...S,background:"#f8fafc"}:S}/>
          ...
          {f.emailVerified && (
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"14px 16px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:12,marginTop:8}}>
              <span style={{fontSize:20}}>✅</span>
              <div><div style={{fontSize:14,fontWeight:600,color:cfg.color}}>Email verified</div><div style={{fontSize:12,color:cfg.color}}>{f.email}</div></div>
            </div>
          )}
        </div>
      )}
      ...
      {step===2 && (
        <div>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:13,fontWeight:600,color:"#374151",display:"block",marginBottom:5}}>Department <span style={{color:"#ef4444"}}>*</span></label>
            <select style={{...S,cursor:"pointer"}} value={f.department} onChange={e=>set("department",e.target.value)}>
              <option value="">Select department</option>
              {DEPTS.map(d=><option key={d} value={d}>{d}</option>)}
            </select>
            {errors.department&&<p style={{fontSize:12,color:"#ef4444",marginTop:4}}>⚠ {errors.department}</p>}
          </div>
          <div style={{marginBottom:14}}><label style={{fontSize:13,fontWeight:600,color:"#374151",display:"block",marginBottom:5}}>Designation <span style={{color:"#ef4444"}}>*</span></label><input style={S} placeholder="e.g. GST Analyst" value={f.designation} onChange={e=>set("designation",e.target.value)}/>{errors.designation&&<p style={{fontSize:12,color:"#ef4444",marginTop:4}}>⚠ {errors.designation}</p>}</div>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:13,fontWeight:600,color:"#374151",display:"block",marginBottom:5}}>Invite Code <span style={{color:"#ef4444"}}>*</span></label>
            <input style={S} placeholder="Code provided by your manager" value={f.inviteCode} onChange={e=>set("inviteCode",e.target.value)}/>
            {errors.inviteCode&&<p style={{fontSize:12,color:"#ef4444",marginTop:4}}>⚠ {errors.inviteCode}</p>}
            <p style={{fontSize:11,color:"#94a3b8",marginTop:4}}>Don't have a code? Contact your admin.</p>
          </div>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:13,fontWeight:600,color:"#374151",display:"block",marginBottom:8}}>Specializations (optional)</label>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {SPECS.map(sp=>(
                <button key={sp} type="button" onClick={()=>toggleSpec(sp)} style={{padding:"6px 14px",fontSize:13,borderRadius:20,border:`1.5px solid ${f.specializations.includes(sp)?cfg.color:"#e2e8f0"}`,background:f.specializations.includes(sp)?cfg.color:"#fff",color:f.specializations.includes(sp)?"#fff":"#64748b",cursor:"pointer"}}>
                  {f.specializations.includes(sp)?"✓ ":""}{sp}
                </button>
              ))}
            </div>
          </div>
      ...
      {step===3 && (
        <div>
          <p style={{fontSize:13,color:"#64748b",marginBottom:14}}>Review your details</p>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {[{l:"Email",v:f.email},{l:"Name",v:`${f.firstName} ${f.lastName}`},{l:"Phone",v:`+91 ${f.phone}`},{l:"Department",v:f.department},{l:"Designation",v:f.designation}].map(({l,v})=>(
